...
from experiments.paper_config import PAPER_SEQ_LEN, PAPER_VOLTAGE_JITTER_V
...
def _cycles_to_tensors(cycle_iter, seq_len=PAPER_SEQ_LEN, rng=None, training_jitter=False):
    features, soh_values, groups = [], [], []
    jitter = PAPER_VOLTAGE_JITTER_V if training_jitter else 0.0

    for item in cycle_iter:
        if len(item) == 5:
            voltage, current, capacity_profile, soh, group_id = item
        else:
            voltage, current, capacity_profile, soh = item
            group_id = "default"
...
def generate_paper_synthetic_data(dataset_name="NASA", num_cycles=150, seq_len=PAPER_SEQ_LEN):
    ...
    base_v = np.linspace(2.5, 4.2, seq_len)
    data = []
    ...
    groups = np.array([f"synthetic_{dataset_name}"] * len(features))
    features, soh_array, groups = prepare_paper_tensors(features, soh_array, groups)
    return features, soh_array, groups
...
class PaperDatasetLoader:
    ...
    def load_dataset(
        ...
        """
        Returns (features, soh, groups).
        groups is a string array of cell/battery ids for grouped CV.
        """
        if raw_path is None:
        ...
        if loaded is not None:
            features, soh, groups = loaded
            features, soh, groups = prepare_paper_tensors(features, soh, groups)
            n_cells = len(set(groups.tolist()))
            print(
                f"[Paper | {dataset_name}] Loaded {len(features)} cycles "
                f"from {n_cells} cells (ICA+DV+DC, grid={seq_len})."
            )
        ...
        (features, soh), groups = generate_paper_synthetic_data(dataset_name, num_cycles, seq_len)
        return features, soh, groups
...
if __name__ == "__main__":
    ...
    for ds in ["NASA", "Oxford", "CALCE"]:
        features, soh, groups = PaperDatasetLoader.load_dataset(ds, num_cycles=10)
        print(f"[{ds}] Features: {features.shape} | SOH: {soh.shape} | cells: {len(set(groups))}")
